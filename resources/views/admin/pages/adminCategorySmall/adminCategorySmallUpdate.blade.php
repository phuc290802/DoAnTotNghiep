        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admincategorysmall.update', $categorysmall->MaTLCon) }}" method="POST">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="col-md-6">
                                    <select id="MaTL" name="MaTL" class="form-control" required>
                                        @foreach($categories as $category)
                                            <option value="{{ $category->MaTL }}" {{ $category->MaTL == $categorysmall->MaTL ? 'selected' : '' }}>
                                                {{ $category->TenTL }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group row">
                              <label for="TenTL" class="col-md-4 col-form-label text-md-right">Tên Thể loại</label>
                              <div class="col-md-6">
                                  <input type="text" id="TenTL" name="TenTL" value="{{ $categorysmall->TenTL }}" class="form-control" required>
                              </div>
                          </div>
